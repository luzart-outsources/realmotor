...
Shader "Mobile/ Realistic Car Shaders/Glass + Diffuse + Decal"
{
	...
	void surf(Input IN, inout SurfaceOutput s)
	{
		...
		body = reflectionResult * (1 - bodySpecularMask) * (1 - decalSpecularMask) + reflectionResult*decalSpecularMask*_DecalReflection + BodyDiffuse * bodySpecularMask* (1 - decalSpecularMask);
		...
		// combine everything
		s.Albedo = _Color*(1 - bodySpecularMask)* (1 - decalSpecularMask) + BodyDiffuse* bodySpecularMask* (1 - decalSpecularMask) + DecalDiffuse * decalSpecularMask + body;
	}
	ENDCG
	}
	FallBack "Standard"
	CustomEditor "SkrilStudio.VehicleGlassDecalBump_Editor"
}